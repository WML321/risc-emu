...
UType_Instruction::UType_Instruction(const std::string& opcode, const int& rd,const int& imm):opcode(opcode),rd(rd),imm(imm) {}

std::string UType_Instruction::to_binary() const{
    std::string imm_bin = std::bitset<20>(this->imm).to_string();
    return imm_bin + std::bitset<5>(this->rd).to_string() + this->opcode;
}

int UType_Instruction::execute(const std::string &rs1_value, const std::string &rs2_value, const std::string &rs3_value) const{
...
std::string UType_Instruction::toString (){
